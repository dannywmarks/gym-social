          ...
          <div >
            <Link href="/">
              <div className="flex items-center gap-2">
                ...
                <p className="capitalize font-medium text-gray-500 hidden text-xs md:block">{post.postedBy.userName}</p>
              </div>
            </Link>
          </div>
        </div>
      ...
      <div className="lg:ml-20 flex gap-4 relative">
        <div className="rounded-3xl" onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
          ...
          {isHover && (<div className="absolute bottom-6 cursor-pointer left-8 md:left-14 lg:left-0 flex gap-10 lg:justify-between w-[100px] md:w-[50px] ">
            ...
            </button> : <button onClick={() => setIsVideoMuted(true)}><HiVolumeUp className="text-black text-2xl lg:text-4xl"/></button>}
          </div>)}
        </div>
      </div>
    </div>
